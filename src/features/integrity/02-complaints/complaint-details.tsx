  ...
  return (
    <>
      ...
      <EntityHeader eyebrow="Complaint registry" title={`Complaint ${complaint.complaintId}`} referenceLabel="Complaint reference" reference={complaint.complaintId} loaded>
        <DetailGrid columns={4}><DetailField label="Complaint ID" value={complaint.complaintId} /><DetailField label="Source type" value={complaint.sourceType} /><DetailField label="Misconduct type" value={complaint.misconductType} /><DetailField label="Date received" value={formatDate(complaint.dateReceived)} /></DetailGrid>
      </EntityHeader>
      <section className="grid grid-cols-1 gap-5 xl:grid-cols-2">
        ...
        <SectionCard title="Linked cases" description="Cases for which this complaint is a recorded source." icon={Link2}><DataTableShell minWidth={520} columns={["Case ID", "Status", "Date opened", "Referral", "Actions"]} rows={(complaint.linkedCases ?? []).map((caseRecord) => ({ key: caseRecord.caseId, cells: [caseRecord.caseId, caseRecord.status, formatDate(caseRecord.dateOpened), caseRecord.referralStatus ?? "—", <Button key="view" asChild size="sm" variant="outline"><Link href={`/integrity/cases/${caseRecord.caseId}`}>View</Link></Button>] }))} emptyTitle="No linked cases found" /></SectionCard>
      </section>
    </>
  );
}
